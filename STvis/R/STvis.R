...
create.seurat.shiny = function(metadata, spatial_img) {
  mat = matrix(1, nrow = 10, ncol = dim(metadata)[1])
  colnames(mat) = rownames(metadata); rownames(mat) = letters[1:10]
  seurat = CreateSeuratObject(mat, assay = "Spatial.shiny", min.cells = 0, min.features = 0, meta.data = metadata)

  image = read_spatial(numBarcode = ifelse(max(seurat$barcodeB) > 50, 96, 50), spatial_img = spatial_img)
  image = image[Cells(seurat)]
  DefaultAssay(image) = "Spatial.shiny"
  seurat[["picture"]] = image
...
server = function(input, output, session) {
  ...
  observeEvent(inFile(), {
    ...
    observeEvent(input$gene.ok, {
      inMat = inMat()
      mat.df = fread(inMat$datapath, select = c("barcode", input$geneInput))
    ...
    output$downloadData = downloadHandler(
      filename = function() {"metadata_shiny.csv"},
      content = function(file) {
        ...
        write.csv(seurat@meta.data, file = file, row.names = T)
      }
    )
  })
  ...
  observeEvent(input$info, {
    showModal(modalDialog(title = "Instructions",
                          HTML("* Copyright (c) 2023.05.12, genger<br>* All rights reserved.<br>",
                               ...
                               "----------------------------------------------------------------------------------------------------------------<br>",
                               "1. 通过[Select feature]选择你想要的feature<br>",
                               "2. 点击图片右上角的[lasso selection]进行拉索选择(圆圈的起点和终点必须相交)<br>",
